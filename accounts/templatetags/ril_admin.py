...
def ril_dashboard_stats():
    ...
    # What the platform keeps on delivered work: each quote minus the expert
    ...
    return {
        ...
        "ready_assign": qs.filter(stage=Stage.PAID, expert__isnull=True).count(),
        ...
        "experts": User.objects.filter(role=User.Role.EXPERT).count(),
